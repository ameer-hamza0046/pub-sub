This is a pub-sub model running on multiple brokers/servers 1 broker/servers servicing multiple subscribers and publishers through a load-balancer which balances the load to the brokers.

To run this do the following steps:
1. First compile the server using:
$ make server
You can then copy the executable on different computers on the same network,
then run those servers.
